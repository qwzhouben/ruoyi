...
//        }
//        return returnList;
        List<SysMenu> nodeList = list.stream()
                .filter(sysMenu -> sysMenu.getParentId() == parentId)
                .map(sysMenu -> createNodeResult(sysMenu, list))
                .collect(Collectors.toList());
        return nodeList;
    }

    private SysMenu createNodeResult(SysMenu sysMenu, List<SysMenu> list) {
        SysMenu node = new SysMenu();
        BeanUtils.copyProperties(sysMenu, node);
        List<SysMenu> children = list.stream()
                .filter(sub -> sub.getParentId()==sysMenu.getMenuId())
                .map(sub -> createNodeResult(sub, list))
                .collect(Collectors.toList());
